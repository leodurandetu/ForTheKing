...
L'affichage du jeu principal est basé sur une grille hexagonale \textit{flat-top} (le haut et le bas plats). Ce format est choisi volontairement pour son rendu esthétique, pour l'adapter au style artistique du projet. 
Pour l'implémentation mathématique de cette grille, la conception s'appuie sur la documentation de référence \textit{Red Blob Games}. La conversion des coordonnées de grille en pixels est assurée par la fonction \emph{case\_vers\_pixels} : les colonnes impaires sont décalées verticalement d'une demi-hauteur, avec un espacement horizontal constant pour assurer l'emboîtement sans trou visible.
...
Afin d'optimiser la lisibilité pendant une action (le personnage se déplace d'une case), la caméra reste centrée en permanence sur le personnage, ce qui permet ainsi un maximum de visibilité sur l'environnement. Un décalage global (\emph{decalage\_x}, \emph{decalage\_y}) est calculé à chaque image et appliqué à l'ensemble de la carte. Ensuite, le rendu suit un ordre strict en couches (tuiles de biome, décor, brouillard, personnage, puis interface). Cette logique d'empilement évite les erreurs visuelles. Le brouillard de guerre est conçu pour encourager l'exploration et est un simple marqueur dans la structure des cases : une fois la zone découverte, le voile se dissipe de manière permanente.

Dès le début du développement, la création d'une bibliothèque d'affichage commune (\emph{affichage\_commun.c}) constitue une priorité. Cette anticipation garantit un code propre et évite la redondance pour les éléments récurrents (\emph{creer\_texte}, \emph{dessiner\_barre}, \emph{dessiner\_inventaire}). 
Enfin, pour livrer une application robuste, le jeu s'adapte dynamiquement à n'importe quelle résolution via \emph{SDL\_GetRendererOutputSize}.
